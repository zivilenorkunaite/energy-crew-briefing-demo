...
Runs with controlled concurrency (3 parallel calls per phase).
...
# Map locations to their primary crew(s) for Genie queries
_LOCATION_CREWS = {
...
DAYS_AHEAD = 10
CONCURRENCY = 3  # parallel calls per batch
...
async def warm_cache() -> AsyncGenerator[dict, None]:
    """Warm the cache. Yields progress dicts for SSE streaming."""
    dates = _get_dates()
    ...
    state = {"done": 0, "skipped": 0, "errors": 0, "total": total}

    def prog(phase, detail=""):
        return {**state, "phase": phase, "detail": detail}

    # ── Helper: run a batch of tasks with concurrency limit ──
    async def run_batch(tasks, phase):
        ...
        for i in range(0, len(tasks), CONCURRENCY * 2):
            ...
            results = await asyncio.gather(*[limited(t) for t in batch], return_exceptions=True)
            for r in results:
    ...
    await run_batch([warm_swms(d) for d in DOCUMENT_NAMES], "swms")
    yield prog("swms", f"SWMS done — {total_swms} documents")

    # ── Phase 2: Weather (19 locations × 11 days) ──
    yield prog("weather", "Loading weather forecasts...")

    async def warm_weather(loc, d):
    ...
    weather_tasks = [warm_weather(loc, d) for loc in _LOCATIONS for d in dates]
    # Yield progress every ~40 tasks
    for i in range(0, len(weather_tasks), 40):
        batch = weather_tasks[i:i + 40]
        sem = asyncio.Semaphore(CONCURRENCY)
        ...
        for r in results:
            if isinstance(r, Exception):
                ...
                state["skipped"] += 1
            state["done"] += 1
        yield prog("weather", f"{state['done']}/{total} — weather")

    # ── Phase 3: Web Search (19 locations) — run in parallel with Genie ──
    yield prog("web+genie", "Loading web searches + Genie queries...")

    async def warm_web(loc):
    ...
    # Combine web + genie into one batch (web is fast, genie is slow)
    web_tasks = [warm_web(loc) for loc in _LOCATIONS]
    genie_tasks = [warm_genie(crew, d)
                   for loc, crews in _LOCATION_CREWS.items()
                   for crew in crews
                   for d in dates]

    all_remaining = web_tasks + genie_tasks
    for i in range(0, len(all_remaining), 20):
        batch = all_remaining[i:i + 20]
        sem = asyncio.Semaphore(CONCURRENCY)
        ...
        for r in results:
            if isinstance(r, Exception):
                ...
                state["skipped"] += 1
            state["done"] += 1
        yield prog("web+genie", f"{state['done']}/{total}")

    yield prog("done", f"Complete — {state['done']} processed, {state['skipped']} cached, {state['errors']} errors")
