...
# MAGIC Refreshes the PAT used by the AI Gateway endpoint.
...
for t in old_tokens:
    if t.get("comment") == "ee-crew-briefing-gateway-auto":
...
resp = requests.post(f"{host}/api/2.0/token/create", headers=headers, json={
    "comment": "ee-crew-briefing-gateway-auto",
...
config = {
    "served_entities": [
        {"external_model": {"name": "databricks-claude-sonnet-4-6", "provider": "databricks-model-serving", "task": "llm/v1/chat",
         "databricks_model_serving_config": {"databricks_workspace_url": host, "databricks_api_token": "{{secrets/ee-crew-briefing/gateway-token}}"}}, "name": "primary-claude"},
        {"external_model": {"name": "databricks-meta-llama-3-3-70b-instruct", "provider": "databricks-model-serving", "task": "llm/v1/chat",
         "databricks_model_serving_config": {"databricks_workspace_url": host, "databricks_api_token": "{{secrets/ee-crew-briefing/gateway-token}}"}}, "name": "fallback-llama"}
    ],
...
resp = requests.put(f"{host}/api/2.0/serving-endpoints/ee-crew-briefing-gateway/config", headers=headers, json=config)
print(f"Gateway config refreshed: {resp.status_code}")

# COMMAND ----------

# Step 5: Verify
resp = requests.post(f"{host}/serving-endpoints/ee-crew-briefing-gateway/invocations", headers=headers,
